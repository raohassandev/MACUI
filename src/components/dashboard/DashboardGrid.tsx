...
const DashboardGrid: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fixResizeHandles = () => {
      ...
      resizeHandles.forEach((handle: any) => {
        ...
        console.log('Fixed resize handle:', handle);
      });
    };
  ...
  const handleLayoutChange = React.useCallback((layout: any) => {
    // Log layout changes to debug resizing issues
  ...
  const handleResizeStart = React.useCallback((_layout: any, oldItem: any, newItem: any, _placeholder: any, _e: any, _element: any) => {
  ...
  const handleResize = React.useCallback((_layout: any, oldItem: any, newItem: any, _placeholder: any, _e: any, _element: HTMLElement) => {
    console.log("Resizing:", { oldItem, newItem, delta: { w: newItem.w - oldItem.w, h: newItem.h - oldItem.h }});
    ...
    if (_element) {
      ...
      if (item) {
        ...
        item.style.setProperty('--actual-width', `${newItem.w * 100 / 12}%`);
  ...
  const handleResizeStop = React.useCallback((_layout: any, oldItem: any, newItem: any, _placeholder: any, _e: any, _element: any) => {
    console.log("Resize stopped:", { oldItem, newItem, finalSize: { w: newItem.w, h: newItem.h }});
