...
import { selectSelectedWidgetId, selectWidget } from '../../redux/slices/widgetSlice';
import { selectCurrentDashboard, updateWidget, removeWidget } from '../../redux/slices/dashboardSlice';
...
const WidgetEditSidebar: React.FC<WidgetEditSidebarProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const loadTags = async () => {
      try {
        ...
        if (Array.isArray(tagsData) && tagsData.length > 0) {
          ...
          // Handle the case where we get empty array
    ...
    // Only run this effect when the sidebar is actually open
    if (isOpen && selectedWidgetId) {
      loadTags();
    }

    // Cleanup function
    return () => {
      // Clean up state when component unmounts or dependency changes
      setIsLoading(false);
    };
  ...
  const handleDelete = () => {
    if (selectedWidgetId) {
      dispatch(removeWidget(selectedWidgetId));
      handleClose();
    }
  };
          ...
          <>
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) => setWidget({ ...widget, chartType: e.target.value as 'line' | 'bar' | 'area' })}
              >
          ...
          <>
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) => setWidget({ ...widget, gaugeType: e.target.value as 'radial' | 'linear' | 'tank' })}
              >
          ...
          <>
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) => setWidget({ ...widget, shape: e.target.value as 'circle' | 'square' | 'pill' })}
              >
            ...
            <div className="mb-4">
              ...
              <div className="bg-gray-100 dark:bg-gray-700 p-2 rounded-md max-h-48 overflow-y-auto">
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>Value</div>
                  <div>Label</div>
                  <div>Color</div>
                </div>
                {widget.statusMap && Object.entries(widget.statusMap).map(([key, value]) => (
  ...
  // If no widget is selected, don't render anything
  if (!widget || !isOpen) {
    return null;
  }
  ...
  return (
    <div 
      className={`fixed right-0 top-0 h-full bg-white dark:bg-gray-800 shadow-xl w-80 transition-transform transform z-40 overflow-hidden ${
      ...
      <div className="flex flex-col h-full">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="mb-4">
                ...
                />
                <span className="text-xs text-gray-500 dark:text-gray-400">
                  Common values: 5s = 5000, 10s = 10000, 1m = 60000
                </span>
              </div>
        ...
        <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          <div className="flex justify-between space-x-2">
            <button
              type="button"
              className="px-3 py-1.5 bg-red-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
            ...
            <div className="flex space-x-2">
              <button
                type="button"
                className="px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600"
                onClick={handleClose}
              >
                Cancel
              </button>
...
export default WidgetEditSidebar;